...
# Cancels ALL active batches (queued or processing jobs)
...
Write-Host "Searching for active batches..." -ForegroundColor Cyan
...
try {
    ...
    Write-Host "⚠️  WARNING: This will cancel ALL these jobs!" -ForegroundColor Red
    ...
    if ($errorMsg -match "404") {
        ...
        Write-Host "`n⚠️  Server error. Check Render logs for batch_id:" -ForegroundColor Yellow
        Write-Host "Look for: 📦 Batch {batch_id} created" -ForegroundColor Gray
        Write-Host "`nThen cancel manually:" -ForegroundColor Cyan
